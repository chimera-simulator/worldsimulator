...
"""
# === Quy ước step-naming (obs.event) theo đúng thứ tự pipeline thật ===
# T0_SEARCH -> T1_CLASSIFY -> T2_SCRAPE -> T2_5_PLANET_GATE ->
# T3_SUMMARIZE -> RULE_LIBRARY_LOAD -> T3_NORMALIZE -> T4_DEDUP ->
# T5_5_LIBRARY_DISTILL -> T5_UPLOAD
# (Vấn đề E — SPEC_ADDENDUM_2_7 mục 6.4: nhãn step trước đây lệch so với
# tên file agent thật, VD step="T4_NORMALIZE" gán cho cả rule_library lẫn
# t3_normalize. Đã fix đồng bộ 1 lần, không sửa lại nữa trừ khi đổi thứ tự
# pipeline thật.)
...
from t2_5_planet_gate import run_planet_gate
...
def _mark_planet_retry_or_fail(
    blackbook: dict,
    working_planet_id: str,
    missing_fields: list,
    cfg=None,
) -> None:
    """Ghi nhận 1 lần REJECT cho working planet hiện tại — dùng CHUNG cho
    cả 2 nguồn REJECT: Gate 5 (missing_required_fields) và T2.5
    insufficient-data (insufficient_fields). retry_count đếm theo SỐ CỬA
    SỔ 25 PHÚT KHÁC NHAU reject cùng 1 working planet (không phân biệt
    gate nào), đúng quyết định kiến trúc đã chốt ở mục 6.1
    SPEC_ADDENDUM_2_7.

    - retry_count >= 3 (current_retry >= 2 trước khi +1) → status =
      "failed_aborted", ghi failed_aborted_log, xoay cursor.
    - Ngược lại → status = "retry_pending", nạp missing_fields vào
      fields_pending, cursor KHÔNG xoay.
    """
    cfg = cfg or config
    rotation = _get_current_planet_archetype(blackbook)
...
def _handle_planet_gate_result(
    ...
    if reject_reason is None:
        ...
        _mark_planet_retry_or_fail(blackbook, working_planet_id, missing_fields, cfg)
...
async def run_pipeline_once(cfg=None) -> dict:
    ...
    try:
        ...
        scraped_docs = await run_scrape_pipeline(
            classified, blackbook, budget=budget, obs=obs,
            working_planet_id=working_planet_id,
            archetype_id=current_planet_archetype_id,
        )
        ...
        # === T2.5: Planet-Scoped Aggregate Gate (SPEC_ADDENDUM_2_7) ===
        gated_docs, gate_report = run_planet_gate(
            scraped_docs, working_planet_id, current_planet_archetype_id,
            blackbook, budget=budget, obs=obs,
        )
        obs.event(step="T2_5_PLANET_GATE", agent="t2_5_planet_gate", status="SUCCESS",
                  items_processed=len(gated_docs),
                  message=f"T2.5 hoàn thành — {len(gated_docs)}/{len(scraped_docs)} document đủ điều kiện gửi T3.")

        if not gate_report["send_to_t3"]:
            _mark_planet_retry_or_fail(
                blackbook, working_planet_id, gate_report["insufficient_fields"], cfg,
            )
            return {"new": 0, "merged": 0, "rejected": 0, "errors": ["planet_gate_insufficient_data"]}

        scraped_docs = gated_docs
        ...
        if rule_check_skipped:
            ...
            obs.event(step="RULE_LIBRARY_LOAD", agent="rule_library", status="WARNING",
        ...
        obs.event(step="T3_NORMALIZE", agent="t3_normalize", status="SUCCESS",
        ...
        obs.event(step="T4_DEDUP", agent="t4_deduplicate", status="SUCCESS",
        ...
        obs.event(step="T5_UPLOAD", agent="t5_upload", status="SUCCESS",
