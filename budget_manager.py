...
class BudgetManager:
    ...
    Quản lý 3 loại tài nguyên cho một chu kỳ Repo 1:
      - max_urls        : tổng URL được phép SCRAPE THẬT (chỉ tính tại t2_scrape,
                          KHÔNG tính URL do t0_search tìm thấy qua parse link).
                          T0 dùng MAX_DISCOVERY_RESULTS_PER_RUN (config.py) riêng.
    ...
    def consume_url(self, count: int = 1) -> bool:
        """Gọi TRƯỚC khi build task scrape thật tại t2_scrape.
        T0 KHÔNG gọi hàm này — discovery không tốn tài nguyên mà max_urls bảo vệ.
        True nếu còn đủ budget, False nếu cạn."""
        with self._lock:
